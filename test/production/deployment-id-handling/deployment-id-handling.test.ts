...
describe.each([
...
])('deployment-id-handling enabled with %s', (envKey) => {
  const deploymentId = String(Date.now())
  const immutableDeploymentId =
    envKey === 'IMMUTABLE_DEPLOYMENT_ID' ? `imm-${deploymentId}` : deploymentId
    ...
    async ({ urlPath }) => {
      ...
      for (const script of scripts) {
        if (script.attribs.src) {
          expect(script.attribs.src).toContain('dpl=' + immutableDeploymentId)
        }
      }
        ...
        if (link.attribs.href && link.attribs.rel !== 'expect') {
          if (link.attribs.as === 'font') {
            expect(link.attribs.href).not.toContain(
              'dpl=' + immutableDeploymentId
            )
          } else {
            expect(link.attribs.href).toContain('dpl=' + immutableDeploymentId)
          }
        }
      }
      ...
      try {
        expect(
          requests.every((item) =>
            item.includes('dpl=' + immutableDeploymentId)
          )
        ).toBe(true)
      } finally {
  ...
  it('should contain deployment id in RSC payload request headers', async () => {
    ...
    expect(
      rscHeaders.every((headers) => headers['x-deployment-id'] === deploymentId)
    ).toBe(true)
  })
})
